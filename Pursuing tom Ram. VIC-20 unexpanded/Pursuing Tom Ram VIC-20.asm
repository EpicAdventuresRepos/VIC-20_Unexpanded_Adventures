...
Init    
        ...
        ; DOll description changes whe the player "travels"
        ; So I have to retart its original description
        ; doll is the fifth object and it will never change
        ; 5x2 = 10 and 11
        ldx #$0
        lda #<desc_doll_0
        sta obj_description,x+$a
        lda #>desc_doll_0
        sta obj_description,x+$b
...
; imprime el mensaje en el INPUT_BUFFER
        ...
        ;lda #<see_object        
        lda INPUT_BUFFER
        JSR PRTSTR ; Este orint me camba la X
...
Ehe_CMD    ; Magical word
        ...
        ; change doll description
        ; now, doll has the same description than candelebra
        ; because it is also a key
        ;ldx #DOLL_IND
        ;dex
        ;txa
        ;asl
        ;tax ; x2 because it is a memory address.
        ldx #$0
        lda #<desc_obj0
        sta obj_description,x+$a
        lda #>desc_obj0
...
F_SO_Loop
        ...
        cmp P0_TMP_BYTE
        bne F_SO_Next_Obj
        ; Object found in the LOC storesd in $fd
...
; Last two bits are the status og the object, but I do not use it.
; I do not use this: 1: 2: 3: Invetopry, 4: Hidden
...
; Tip: early versions had a CHandelier, but I chaged it for a CAndelebra
...
desc_doll_0
        TEXT    "it listened and" 
        BYTE    RETURN
        TEXT    "comforted the children" 
        BYTE    $0
...
LOC_MONSTERS = $b
LOC_PAINTS = $c
LOC_INVENTORY=%00111111 ; LOC of the items in inventory (3F)
LOC_HIDDEN = %00111110 ; LOC of hidden objects(3F)
LOC_DESTROYED = %00111101 ; LOC of objects will never appear in game
...
desc12 
        TEXT "hall ofpaints" ; Dont fix the space
        BYTE    $0
...
str_open_trapdoor
        ...
        TEXT "you see a hole where"
        BYTE    RETURN
        TEXT "tom locked up the"
        BYTE    RETURN
        TEXT "kidnapped children"
        BYTE    RETURN, $0
